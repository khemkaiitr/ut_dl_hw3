...
class SuperTuxDataset(Dataset):
    def __init__(self, dataset_path, transform=None):
        ...
        """
        self.img_dir = dataset_path
        self.data = []
        self.label_file = os.path.join(self.img_dir, 'labels.csv')
        ...
        with open(self.label_file, 'r') as csv_file:
            ...
            for fname, label, _ in reader:
                if label in LABEL_NAMES:
                    ...
                    self.data.append((tensor(image), label_id))
    ...
    def __getitem__(self, idx):
        ...
        #
        #
        # img_path = os.path.join(self.img_dir, self.label[idx][0])
        # img = Image.open(img_path)
        # transform = transforms.ToTensor()
        # img_tensor = transform(img)
        # label = LABEL_DICT[self.label[idx][1]]
